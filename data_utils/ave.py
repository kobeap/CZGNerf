...
if __name__ == '__main__':
    ...
    ckpt = torch.load('./data_utils/ave_checkpoint/audio_visual_encoder.pth')
    model.load_state_dict({f'audio_encoder.{k}': v for k, v in ckpt.items()})
    print('start dealing')
    dataset = AudDataset(wav_name)
    ...
    # print(outputs.shape) torch.Size([64, 1, 80, 16])
    first_frame, last_frame = outputs[:1], outputs[-1:]
    aud_features = torch.cat([first_frame.repeat(2, 1), outputs, last_frame.repeat(2, 1)], dim=0).numpy()
    print('aud_features',aud_features.shape)    # (2999,512)
    np.save(wav_name.replace('.wav', '_ave.npy'), aud_features)