...
class _march_rays_train(Function):
    ...
    def forward(ctx, rays_o, rays_d, bound, density_bitfield, C, H, nears, fars, step_counter=None, mean_count=-1, perturb=False, align=-1, force_all_rays=False, dt_gamma=0, max_steps=1024):
        ...
        Returns:
            xyzs: float, [M, 3], all generated points' coords. (all rays concated, need to use `rays` to extract points belonging to each ray)
                所有生成点的坐标，尺寸为 [M, 3]，M 是所有光线生成的点的总数。使用 `rays` 来提取属于每条光线的点。
            dirs: float, [M, 3], all generated points' view dirs.
                所有生成点的视角方向，尺寸为 [M, 3]。
            deltas: float, [M, 2], first is delta_t, second is rays_t
                变化量，尺寸为 [M, 2]，第一个是时间步长，第二个是光线的时间。
            rays: int32, [N, 3], all rays' (index, point_offset, point_count), e.g., xyzs[rays[i, 1]:rays[i, 1] + rays[i, 2]] --> points belonging to rays[i, 0]
                所有光线的信息，尺寸为 [N, 3]，每行表示一条光线的索引、点偏移和点计数。
        '''
...
class _composite_rays_train_triplane(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, rgbs, amb_aud, amb_eye,amb_emo, uncertainty, deltas, rays, T_thresh=1e-4):
        ''' composite rays' rgbs, according to the ray marching formula.
        Args:
            rgbs: float, [M, 3]
            sigmas: float, [M,]
        ...
        Returns:
            ...
            image: float, [N, 3], the RGB channel (after multiplying alpha!)
        '''
        # 确保所有输入张量在内存中是连续的，以提高计算效率。
        sigmas = sigmas.contiguous()
        rgbs = rgbs.contiguous()
        amb_aud = amb_aud.contiguous()
        amb_eye = amb_eye.contiguous()

        amb_emo = amb_emo.contiguous()

        uncertainty = uncertainty.contiguous()
        # M 是 sigmas 的长度，相当于采样点数量
        # N 是 rays 的长度。相当于采样的像素点数量
        M = sigmas.shape[0]
        N = rays.shape[0]
        # 创建用于存储结果的空张量，这些张量的类型和设备与 sigmas 相同。形状是[N]
        weights_sum = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        ...
        image = torch.empty(N, 3, dtype=sigmas.dtype, device=sigmas.device)
        # TODO 体渲染调用函数
        amb_emo_sum = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        # 调用后端函数 composite_rays_train_triplane_forward 进行实际的光线合成计算，并将结果存储在上述创建的张量中。
        # amb_emo [M,1] or None ,amb_emo_sum torch.empty[N]
        # 如果amb_emo是None,那么不会对amb_emo_sum进行修改
        _backend.composite_rays_train_triplane_forward(sigmas, rgbs, amb_aud, amb_eye,amb_emo,uncertainty, deltas, rays, M, N, T_thresh, weights_sum, amb_aud_sum, amb_eye_sum,amb_emo_sum, uncertainty_sum, depth, image)
        # TODO 存储用于反向传播的数据
        ctx.save_for_backward(sigmas, rgbs, amb_aud, amb_eye,amb_emo, uncertainty, deltas, rays, weights_sum, amb_aud_sum, amb_eye_sum,amb_emo_sum, uncertainty_sum, depth, image)
        ctx.dims = [M, N, T_thresh]

        return weights_sum, amb_aud_sum, amb_eye_sum,amb_emo_sum, uncertainty_sum, depth, image
    
    @staticmethod
    @custom_bwd
    def backward(ctx, grad_weights_sum, grad_amb_aud_sum, grad_amb_eye_sum,grad_amb_emo_sum,grad_uncertainty_sum, grad_depth, grad_image):
        """
        这些参数是根据 forward 函数的输出确定的。在 forward 函数中，有多少个输出，backward 中就会有相应数量的梯度参数。ctx 对象用于在 forward 和 backward 之间传递信息。

        """

        # NOTE: grad_depth is not used now! It won't be propagated to sigmas.
        # print('wwwwwwwwwww')
        # print(grad_amb_emo_sum.shape)  [65536]
        grad_weights_sum = grad_weights_sum.contiguous()
        grad_amb_aud_sum = grad_amb_aud_sum.contiguous()
        grad_amb_eye_sum = grad_amb_eye_sum.contiguous()

        grad_amb_emo_sum = grad_amb_emo_sum.contiguous()

        grad_uncertainty_sum = grad_uncertainty_sum.contiguous()
        grad_image = grad_image.contiguous()

        sigmas, rgbs, amb_aud, amb_eye, amb_emo,uncertainty, deltas, rays, weights_sum, amb_aud_sum, amb_eye_sum, amb_emo_sum,uncertainty_sum, depth, image = ctx.saved_tensors
        M, N, T_thresh = ctx.dims
        ...
        grad_amb_emo = torch.zeros_like(amb_emo)

        grad_uncertainty = torch.zeros_like(uncertainty)
        # print(11111111)
        _backend.composite_rays_train_triplane_backward(grad_weights_sum, grad_amb_aud_sum, grad_amb_eye_sum, grad_amb_emo_sum,grad_uncertainty_sum, grad_image, sigmas, rgbs, amb_aud, amb_eye,amb_emo, uncertainty, deltas, rays, weights_sum, amb_aud_sum, amb_eye_sum,amb_emo_sum, uncertainty_sum, image, M, N, T_thresh, grad_sigmas, grad_rgbs, grad_amb_aud, grad_amb_eye, grad_amb_emo,grad_uncertainty)
        # print('111111111111111111111111111')
        # print(grad_amb_emo.shape)  [343808]
        # print(grad_amb_eye.shape,11111)
        return grad_sigmas, grad_rgbs, grad_amb_aud, grad_amb_eye,grad_amb_emo, grad_uncertainty, None, None, None
...
class _composite_rays_triplane(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32) # need to cast sigmas & rgbs to float
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, ambs_aud, ambs_eye,ambs_emo, uncertainties, weights_sum, depth, image, amb_aud_sum, amb_eye_sum,amb_emo_sum, uncertainty_sum, T_thresh=1e-2):
        _backend.composite_rays_triplane(n_alive, n_step, T_thresh, rays_alive, rays_t, sigmas, rgbs, deltas, ambs_aud, ambs_eye,ambs_emo, uncertainties, weights_sum, depth, image, amb_aud_sum, amb_eye_sum,amb_emo_sum, uncertainty_sum)
        return tuple()
