...
// -------------------------------- triplane -----------------------------
// 这段代码是一个CUDA核函数，用于在GPU上并行执行三平面(triplane)体积渲染的前向传播计算。该函数主要完成以下任务：
// 1.对每条光线的体积密度、颜色、环境音频、环境视觉、和不确定性进行累积计算。
// 2.根据每步的透明度(alpha)和透射率(transmittance)来更新这些累积值。
// 3.当光线的透射率低于阈值时，终止累积过程。
// sigmas: [M]
// rgbs: [M, 3]
// deltas: [M, 2]
// rays: [N, 3], idx, offset, num_steps
...
__global__ void kernel_composite_rays_train_triplane_forward(
    ...
    const scalar_t * __restrict__ amb_emo,
    ...
    scalar_t * amb_emo_sum,

    scalar_t * uncertainty_sum,
    scalar_t * depth,
    scalar_t * image
) {
    // parallel per ray   并行处理每条光线
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    if (n >= N) return;

    // locate   获取光线信息  offset
    uint32_t index = rays[n * 3];
    uint32_t offset = rays[n * 3 + 1];
    uint32_t num_steps = rays[n * 3 + 2];

    // empty ray, or ray that exceed max step count. 如果步数为0或步数超出范围，直接将对应输出值设为0并返回。
    if (num_steps == 0 || offset + num_steps > M) {
        weights_sum[index] = 0;
        amb_aud_sum[index] = 0;
        amb_eye_sum[index] = 0;
    ...
    // 这一步的目的是通过增加指针的偏移量，将指针移动到特定的内存位置，以便在后续操作中处理从 offset 开始的一段数据。
    sigmas += offset;
    rgbs += offset * 3;
    amb_aud += offset;
    amb_eye += offset;
    ...
    while (step < num_steps) {
        ...
        // minimal remained transmittence  更新透射率 T，如果透射率低于阈值则终止循环。
        if (T < T_thresh) break;
...
void composite_rays_train_triplane_forward(const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor amb_aud, const at::Tensor amb_eye, const at::Tensor amb_emo, const at::Tensor uncertainty, const at::Tensor deltas, const at::Tensor rays, const uint32_t M, const uint32_t N, const float T_thresh, at::Tensor weights_sum, at::Tensor amb_aud_sum, at::Tensor amb_eye_sum, at::Tensor amb_emo_sum, at::Tensor uncertainty_sum, at::Tensor depth, at::Tensor image) {

    static constexpr uint32_t N_THREAD = 128;
    // 判断是否使用情绪损失
    // const float* amb_emo_ptr = amb_emo.defined() ? amb_emo.data_ptr<float>() : nullptr;
    // float * amb_emo_ptr = amb_emo.defined() ? amb_emo.data_ptr<scalar_t>() : nullptr;
    // 函数调用
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    sigmas.scalar_type(), "composite_rays_train_triplane_forward", ([&] {
        kernel_composite_rays_train_triplane_forward<<<div_round_up(N, N_THREAD), N_THREAD>>>(sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), amb_aud.data_ptr<scalar_t>(), amb_eye.data_ptr<scalar_t>(),amb_emo.data_ptr<scalar_t>() , uncertainty.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(), M, N, T_thresh, weights_sum.data_ptr<scalar_t>(), amb_aud_sum.data_ptr<scalar_t>(), amb_eye_sum.data_ptr<scalar_t>(),amb_emo_sum.data_ptr<scalar_t>(), uncertainty_sum.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>());
    }));
}
...
__global__ void kernel_composite_rays_train_triplane_backward(
    ...
    const scalar_t * __restrict__ grad_amb_emo_sum,
    ...
    const scalar_t * __restrict__ amb_emo_sum,
    ...
    while (step < num_steps) {
        ...
        grad_amb_emo[0] = grad_amb_emo_sum[0];
...
void composite_rays_train_triplane_backward(const at::Tensor grad_weights_sum, const at::Tensor grad_amb_aud_sum, const at::Tensor grad_amb_eye_sum,const at::Tensor grad_amb_emo_sum, const at::Tensor grad_uncertainty_sum, const at::Tensor grad_image, const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor amb_aud, const at::Tensor amb_eye,const at::Tensor amb_emo, const at::Tensor uncertainty, const at::Tensor deltas, const at::Tensor rays, const at::Tensor weights_sum, const at::Tensor amb_aud_sum, const at::Tensor amb_eye_sum, const at::Tensor amb_emo_sum, const at::Tensor uncertainty_sum, const at::Tensor image, const uint32_t M, const uint32_t N, const float T_thresh, at::Tensor grad_sigmas, at::Tensor grad_rgbs, at::Tensor grad_amb_aud, at::Tensor grad_amb_eye, at::Tensor grad_amb_emo,at::Tensor grad_uncertainty) {

    static constexpr uint32_t N_THREAD = 128;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    grad_image.scalar_type(), "composite_rays_train_triplane_backward", ([&] {
        kernel_composite_rays_train_triplane_backward<<<div_round_up(N, N_THREAD), N_THREAD>>>(grad_weights_sum.data_ptr<scalar_t>(), grad_amb_aud_sum.data_ptr<scalar_t>(), grad_amb_eye_sum.data_ptr<scalar_t>(), grad_amb_emo_sum.data_ptr<scalar_t>(), grad_uncertainty_sum.data_ptr<scalar_t>(), grad_image.data_ptr<scalar_t>(), sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), amb_aud.data_ptr<scalar_t>(), amb_eye.data_ptr<scalar_t>(), amb_emo.data_ptr<scalar_t>(), uncertainty.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(), weights_sum.data_ptr<scalar_t>(), amb_aud_sum.data_ptr<scalar_t>(), amb_eye_sum.data_ptr<scalar_t>(), amb_emo_sum.data_ptr<scalar_t>(), uncertainty_sum.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), M, N, T_thresh, grad_sigmas.data_ptr<scalar_t>(), grad_rgbs.data_ptr<scalar_t>(), grad_amb_aud.data_ptr<scalar_t>(), grad_amb_eye.data_ptr<scalar_t>(), grad_amb_emo.data_ptr<scalar_t>(), grad_uncertainty.data_ptr<scalar_t>());
    }));
}
...
__global__ void kernel_composite_rays_triplane(
    ...
    const scalar_t* __restrict__ ambs_emo,

    const scalar_t* __restrict__ uncertainties, 
    scalar_t* weights_sum, scalar_t* depth, scalar_t* image, scalar_t* amb_aud_sum, scalar_t* amb_eye_sum,scalar_t* amb_emo_sum, scalar_t* uncertainty_sum
) {
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    if (n >= n_alive) return;
...
void composite_rays_triplane(const uint32_t n_alive, const uint32_t n_step, const float T_thresh, at::Tensor rays_alive, at::Tensor rays_t, at::Tensor sigmas, at::Tensor rgbs, at::Tensor deltas, at::Tensor ambs_aud, at::Tensor ambs_eye, at::Tensor ambs_emo,at::Tensor uncertainties, at::Tensor weights, at::Tensor depth, at::Tensor image, at::Tensor amb_aud_sum, at::Tensor amb_eye_sum,at::Tensor amb_emo_sum, at::Tensor uncertainty_sum) {
    static constexpr uint32_t N_THREAD = 128;
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    image.scalar_type(), "composite_rays_triplane", ([&] {
        kernel_composite_rays_triplane<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(n_alive, n_step, T_thresh, rays_alive.data_ptr<int>(), rays_t.data_ptr<scalar_t>(), sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), ambs_aud.data_ptr<scalar_t>(), ambs_eye.data_ptr<scalar_t>(),ambs_emo.data_ptr<scalar_t>(), uncertainties.data_ptr<scalar_t>(), weights.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), amb_aud_sum.data_ptr<scalar_t>(), amb_eye_sum.data_ptr<scalar_t>(),amb_emo_sum.data_ptr<scalar_t>(), uncertainty_sum.data_ptr<scalar_t>());
    }));
}
