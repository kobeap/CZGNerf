...
class NeRFRenderer(nn.Module):
    ...
    def run_cuda(self, rays_o, rays_d, auds, bg_coords, poses, emo_label=None, eye=None, index=0, dt_gamma=0,
                 bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # auds: [B, 16]
        # eye [B,1] emo_label[B,1]
        # index: [B]
        ...
        if self.training:
            ...
            xyzs, dirs, deltas, rays = raymarching.march_rays_train(rays_o, rays_d, self.bound, self.density_bitfield,
                                                                    ...
                                                                    dt_gamma, max_steps)
            # print(xyzs.shape)
            # if emo_label is None ,amb_emo is None ,else is [B,1]
            # print(emo_label.shape,111111) # [1,1]
            sigmas, rgbs, amb_aud, amb_eye, amb_emo, uncertainty = self(xyzs, dirs, enc_a, ind_code, eye, emo_label)
            sigmas = self.density_scale * sigmas
            # print(self.training)

            # #print(f'valid RGB query ratio: {mask.sum().item() / mask.shape[0]} (total = {mask.sum().item()})')

            # weights_sum, ambient_sum, uncertainty_sum, depth, image = raymarching.composite_rays_train_uncertainty(sigmas, rgbs, ambient.abs().sum(-1), uncertainty, deltas, rays)     amb_eye[N,1]
            weights_sum, amb_aud_sum, amb_eye_sum, amb_emo_sum, uncertainty_sum, depth, image = raymarching.composite_rays_train_triplane(
                sigmas, rgbs, amb_aud.abs().sum(-1), amb_eye.abs().sum(-1), amb_emo.abs().sum(-1), uncertainty, deltas,
                rays)
            ...
            results['ambient_emo'] = amb_emo_sum

            results['rays'] = xyzs, dirs, enc_a, ind_code, eye
            results["emo_label"] = emo_label
        else:
            ...
            amb_emo_sum = torch.zeros(N, dtype=dtype, device=device)
            ...
            while step < max_steps:
                ...
                sigmas, rgbs, ambients_aud, ambients_eye, ambients_emo, uncertainties = self(xyzs, dirs, enc_a,
                                                                                             ind_code, eye, emo_label)
                # print(ambients_emo)
                sigmas = self.density_scale * sigmas
                ...
                raymarching.composite_rays_triplane(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas,
                                                    ambients_aud, ambients_eye, ambients_emo, uncertainties,
                                                    weights_sum, depth, image, amb_aud_sum, amb_eye_sum, amb_emo_sum,
                                                    uncertainty_sum, T_thresh)
    ...
    def run_torso(self, rays_o, bg_coords, poses, index=0, bg_color=None, **kwargs):
        ...
        if self.torso:
            ...
            occupancy = F.grid_sample(self.density_grid_torso.view(1, 1, self.grid_size, self.grid_size),
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        if not self.torso:  # forbid updating head if is training torso...
            ...
            if self.exp_eye:
                ...
                eye = None
            if self.emo:
                emo = self.emo_labels[[rand_idx]].to(self.density_bitfield.device)  # [1, 1]
            else:
                emo = None
            # full update
            X = torch.arange(self.grid_size, dtype=torch.int32, device=self.density_bitfield.device).split(S)
                ...
                for ys in Y:
                    for zs in Z:
                        ...
                        for cas in range(self.cascade):
                            ...
                            # print(cas_xyzs.shape)
                            # query density
                            sigmas = self.density(cas_xyzs, enc_a, eye, emo_label=emo)['sigma'].reshape(-1).detach().to(
                                tmp_grid.dtype)
            ...
            for ys in Y:
                for zs in Z:
                    ...
                    for cas in range(self.cascade):
                        ...
                        cas_xyzs += (torch.rand_like(cas_xyzs) * 2 - 1) * half_grid_size
                        # query density
                        aud_norms = self.density(cas_xyzs.to(tmp_grid.dtype), enc_a, eye)['ambient_aud'].reshape(
                            -1).detach().to(tmp_grid.dtype)
                        # assign
            ...
            for ys in Y:
                for zs in Z:
                    ...
                    for cas in range(self.cascade):
                        ...
                        cas_xyzs += (torch.rand_like(cas_xyzs) * 2 - 1) * half_grid_size
                        # query density
                        eye_norms = self.density(cas_xyzs.to(tmp_grid.dtype), enc_a, eye)['ambient_eye'].reshape(
                            -1).detach().to(tmp_grid.dtype)
                        # assign
