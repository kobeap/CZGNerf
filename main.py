...
# pydevd_pycharm.settrace('10.24.3.69', port=21000, stdoutToServer=True, stderrToServer=True)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--asr_model', type=str, default='deepspeech',help="ave")
    ...
    # TODO  Perceptual Loss Functions
    #  Generic loss function options # 目前用的只有content loss, TV loss
    parser.add_argument('--tv_strength', default=1e-6)
    parser.add_argument('--use_tv_loss', default=True)
    ...
    parser.add_argument("--content_layers", type=str, default="8", help="图片内容特征提取的层的索引序列,默认8")
    parser.add_argument("--content_size", type=int, default=512, help="训练图片的大小，默认为 512*512")
    # vgg model
    ...
    # TODO 改进所添加使用的参数
    parser.add_argument('--asr_model', type=str, default='hubert', help="ave")
    parser.add_argument('--use_percep_loss', default=True, help="True,False")
    parser.add_argument('--content_weight', type=float, default=0.001)
    parser.add_argument("--tornet", default=False, help='是否用编码器替代躯干变形mlp,False是原本的mlp True是更改的网络')
    # resnet网络进行注意力优化
    parser.add_argument('--resnet', type=str, default='LineModel', help='LineModel,MWSE,MWDN,None')

    parser.add_argument('--use_atan', default=False,
                        help='是否应用transformer网络在提取音频注意力机制,有AudTransAttNet,AudioMambaModel，False三个选择')
    parser.add_argument('--portrait', action='store_true', help="only render face")
    # 眨眼或者表情控制
    parser.add_argument('--bs_area', type=str, default="upper", help="upper or eye,single")
    parser.add_argument('--au45', default=False ,action='store_true', help="True or False")

    # 音频
    parser.add_argument('--video_aud', default='', type=str ,help="video_audio")

    # 情绪标签
    parser.add_argument('--emo', default=True,action='store_true', help="情绪标签，True or False")
    parser.add_argument('--exp_emo', action='store_true', help="explicitly control the face")
    parser.add_argument('--amb_emo_loss',type=int, default=1, help="use emo_loss or not")
    parser.add_argument('--emo_lstm',default=False, help="与情绪控制相关的lstm")

    # 对比学习
    parser.add_argument('--contrast_loss',default=True)
    parser.add_argument('--rev_aud', default='/mnt/data1/zpf/ernerf/cnn_folder/cnn_openface_flow/aud_ave.npy')

    opt = parser.parse_args()

    # --workspace /mnt/data1/zpf/ernerf/e_w_hair_torso/add_loss_trial_e_w_torso -O --torso --iters 125000 --head_ckpt /mnt/data1/zpf/ernerf/e_w_hair_torso/add_loss_trial_e_w/checkpoints/ngp_ep0083.pth --iters 200000 --asr_model hubert
    # 配置结束
    ...
    # TODO
    # 内容重建误差和风格误差 dafeng版
    content_loss_func, style_loss_func = perceptual_functions.ContentLoss(
        opt.content_weight), perceptual_functions.StyleLoss(opt.style_weight)
    perceptual_loss_func = (content_loss_func, style_loss_func)
    tv_loss = tvloss.TVLoss()
    ...
    if opt.test:
        ...
        trainer = Trainer('ngp', opt, model, device=device, workspace=opt.workspace, criterion=criterion, fp16=opt.fp16,
                          metrics=metrics, use_checkpoint=opt.ckpt, perceptual_loss_func=perceptual_loss_func,
                          tv_loss=tv_loss)
        ...
        trainer = Trainer('ngp', opt, model, device=device, workspace=opt.workspace, optimizer=optimizer,
                          ...
                          perceptual_loss_func=perceptual_loss_func, tv_loss=tv_loss)
